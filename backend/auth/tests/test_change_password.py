...
def test_login_with_outdated_password(api_client, create_user):
    ...
    assert old_pw_resp.status_code == 400, "Did not expect successful login with old password"
...
def test_change_password_with_wrong_current(api_client, create_user):
    ...
    assert resp.status_code == 400, "Did not expect successful password change"
